...
export async function executeHook(
	...
	if (hook.kind === 'prompt' || hook.kind === 'agent') {
		return executePromptHook(hook, payload)
	}
	return {
		hook,
		success: false,
...
function executeCommandHook(hook: CompatHook, payload: Record<string, unknown>, cwd: string): Promise<HookResult> {
	if (!hook.command) {
	...
	return new Promise(resolve => {
		...
		proc.stdin?.end(sanitizePayloadObject(payload))
		...
		proc.on('close', code => {
			...
			const stdout =
				Buffer.concat(stdoutChunks).toString('utf-8').trim() + (stdoutTruncated ? '\n[output truncated]' : '')
			const stderr =
				Buffer.concat(stderrChunks).toString('utf-8').trim() + (stderrTruncated ? '\n[output truncated]' : '')
...
function executePromptHook(hook: CompatHook, payload: Record<string, unknown>): Promise<HookResult> {
	if (!hook.prompt) {
		return Promise.resolve({
			hook,
			success: false,
	...
	return Promise.resolve({
		...
		success: true,
		blocked: false,
		output: injectArguments(hook.prompt, payload),
	})
}

async function executeHttpHook(hook: CompatHook, payload: Record<string, unknown>): Promise<HookResult> {
	if (!hook.url) {
		return {
			hook,
			success: false,
...
function sanitizePayload(payload: Record<string, unknown>): string {
	return JSON.stringify(sanitizePayloadValue(payload))
}

function sanitizePayloadObject(payload: Record<string, unknown>): string {
	return `${sanitizePayload(payload)}\n`
}

function sanitizePayloadValue(payload: Record<string, unknown>): Record<string, unknown> {
...
function isSafeCommand(command: string): boolean {
	const blocked = [/\brm\s+-rf\s+\//, /\b(?:wget|curl)\s+.*\|\s*sh/, /\bdd\s+if=/, />\s*\/dev\/sd/, /\bchmod\s+777/]
