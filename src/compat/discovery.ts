...
import type { LoadedCompatPlugin, PluginDiagnostic, PluginConfig, CompatibilityReport } from '../shared/types.js'

export function discoverPluginRoots(cwd: string, config?: PluginConfig): string[] {
	const roots: string[] = [join(homedir(), '.openharness', 'plugins'), join(cwd, '.openharness', 'plugins')]
...
export function loadPluginsFromDirs(pluginDirs: string[], config?: PluginConfig, cwd?: string): LoadedCompatPlugin[] {
	...
	for (const dir of pluginDirs) {
		...
		const commands = shouldLoadArtifacts ? loadCommandsFromPlugin(dir, manifest.name, manifest.commands) : []
		const agents = shouldLoadArtifacts ? loadAgentsFromPlugin(dir, manifest.name, manifest.agents, diagnostics) : []
